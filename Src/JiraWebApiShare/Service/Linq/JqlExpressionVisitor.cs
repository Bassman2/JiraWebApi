...
internal class JqlExpressionVisitor: ExpressionVisitor 
{
    ...
    private readonly StringBuilder jql;
    ...
    private static JqlFieldAttribute? GetFieldInfo(Expression expression)
    {
        ...
        case ExpressionType.MemberAccess:
            {
                ...
                {
                    return propertyInfo.GetCustomAttribute<JqlFieldAttribute>();
                }
        ...
        case ExpressionType.Convert:
            {
                if (expression is UnaryExpression unaryExpression && unaryExpression.Operand is MemberExpression memberExpression && memberExpression.Member is PropertyInfo propertyInfo)
                {
                    return propertyInfo.GetCustomAttribute<JqlFieldAttribute>();
                }
                throw new Exception("Unknown error!");
        ...
        case ExpressionType.Call:
            {
                string name = "unknown";
                if (expression is MethodCallExpression methodCallExpression && methodCallExpression.Arguments[0] is ConstantExpression constantExpression)
                {
    ...
    {
        var args = node.Arguments.Select(static e => ((ConstantExpression)e).Value);
    ...
    protected override Expression VisitMember(MemberExpression node)
    {
        if (node.Member is PropertyInfo propertyInfo)
        {
            ...
            JqlFieldAttribute field = (JqlFieldAttribute)attributes[0];
            this.jql.Append(field.Name);
        }
        else
        {
