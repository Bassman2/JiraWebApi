...
public class JiraIssueTypeUnitTest : JiraBaseUnitTest
{
    ...
    public async Task TestMethodGetIssueTypesAsync()
    {
        ...
        Assert.AreEqual($"{testHost}rest/api/2/issuetype/1", issueType.Self, nameof(issueType.Self));
        ...
        Assert.AreEqual($"{testHost}secure/viewavatar?size=xsmall&avatarId=27303&avatarType=issuetype", issueType.IconUrl?.ToString(), nameof(issueType.IconUrl));
